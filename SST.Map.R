...
# reclassify SST
summary(SST$anom)
brks <- c(min(SST$anom),-4,-3,-2,-1.5,-1,-.5,-.1,.1,.5,1,1.5,2,3,4,max(SST$anom))
SST$reclass <- cut(SST$anom, breaks = brks, include.lowest = T)


# sst colours 
red.pal <- colorRampPalette(c("#ffffff", "#FEE64C" , "#FC702A",  "#D9000B"), 
                            bias = 1.2, space = c("rgb", "Lab"))(8)
blue.pal <- colorRampPalette(c("#0B57DE", "#2EA0F2","#4ACCFF", "#ffffff"), 
                             bias = 1.2, space = c("rgb", "Lab"))(8)
pals <- c(blue.pal[1:8], red.pal[2:8])

pal <- data.frame(bin = sort(unique(SST$reclass)), cols=pals)
pal$cols <- as.character(pal$cols)
pal$labs <- c("<-4", " -4", " -3", " -2", " -1.5", " -1", " -0.5", "  0", 
              "  0.5",  "  1", "  1.5", "  2", "  3", "  4", "> 4")

#sst dataset with colours
anon <- merge(SST,pal, by.x="reclass",by.y="bin")
...
for (i in 1:length(plots)){
  ...
  # Save as a pdf
  pdf(paste("Figures/SST/Maps/SSTanom.1998-2015.Map",plots[i],"pdf",sep="."), 
      width=7, height=6) 
  print(sstPlot)
  dev.off()
...
# load sst raster
load("Data/SST.NPO_Raster.1998-2015.qd.rda") # SST

# reclassify SST
SST$reclass <- round_any(SST$Temp, 2)
sort(unique(round(SST$reclass)))
length(unique(round(SST$reclass)))
...
sst <- merge(SST,pal, by.x="reclass",by.y="labs")
...
for (i in 1:length(plots)){
  ...
  dat <- sst[sst$Month == mths[i],]
  
  name <- paste(plots[i]," SST", sep="")
  ...
  # Save as a pdf
  pdf(paste("Figures/SST/Maps/SST.1998-2015.Map",plots[i],"pdf",sep="."), 
      width=7, height=6) 
  print(sstPlot)
  dev.off()
